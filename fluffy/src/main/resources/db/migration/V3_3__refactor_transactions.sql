...
-- transaction_type と transfer_id の整合性を保証する CHECK 制約を追加
-- （TRANSFER は必ず transfer_id を持ち、それ以外は持たない）
ALTER TABLE transactions
    ADD CONSTRAINT chk_transaction_type_transfer_id CHECK (
        (transaction_type = 'TRANSFER' AND transfer_id IS NOT NULL)
        OR
        (transaction_type IN ('NORMAL', 'ADJUSTMENT') AND transfer_id IS NULL)
    );
