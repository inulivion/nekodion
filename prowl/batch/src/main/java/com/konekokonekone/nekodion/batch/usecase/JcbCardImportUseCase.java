...
public class JcbCardImportUseCase {
    ...
    private static final String GMAIL_QUERY_TEMPLATE = "subject:\"JCBカード／ショッピングご利用のお知らせ\" after:%s";
    ...
    public void execute(User user, Account account, BatchResult result) {
        ...
        for (GmailMessage message : messages) {
            ...
            try {
                ...
                log.error("JCBカードメールの処理に失敗しました。messageId={}", message.getId(), e);
                result.add(BatchResultStatus.FAILURE);
            }
        }
    }
