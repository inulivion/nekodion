...
public class SmbcCardImportUseCase {
    ...
    private static final String GMAIL_QUERY_TEMPLATE = "subject:\"ご利用のお知らせ【三井住友カード】\" after:%s";
    ...
    private static final Pattern DATE_PATTERN = Pattern.compile("◇?利用日：(\\d{4}/\\d{2}/\\d{2} \\d{2}:\\d{2})");

    private static final Pattern SHOP_PATTERN = Pattern.compile("◇?利用先：(.+)");

    private static final Pattern AMOUNT_PATTERN = Pattern.compile("◇?利用金額：([\\d,.]+)(?:円|JPY)");
    ...
    public void execute(User user, Account account, BatchResult result) {
        ...
        for (GmailMessage message : messages) {
            ...
            try {
                ...
                log.error("三井住友カードメールの処理に失敗しました。messageId={}", message.getId(), e);
                result.add(BatchResultStatus.FAILURE);
            }
        }
    }
